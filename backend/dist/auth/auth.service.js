...
let AuthService = class AuthService {
    ...
    async register(dto) {
        if (!dto.email && !dto.phone) {
            throw new common_1.BadRequestException('邮箱或手机号至少填写一个');
        }
        ...
        if (exists)
            throw new common_1.ConflictException('该账号已注册，请直接登录');
        ...
        const user = await this.prisma.user.create({
            data: {
                ...
                phone: dto.phone,
                password: hash,
                nickname: dto.nickname || dto.email?.split('@')[0] || dto.phone || '旅行者',
            },
        });
    ...
    buildAuthResult(user, guest = false) {
