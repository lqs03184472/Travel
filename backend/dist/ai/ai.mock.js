...
exports.buildMockPlans = buildMockPlans;
...
function genericDest(dest, days) {
    const items = [];
    const firstDay = [
        { time: '08:00', type: 'transport_main', title: `出发地→${dest}（高铁/飞机）`, cost: 200, cat: 'transport', detail: '首选高铁，出站后打车/公交到市区' },
        { time: '14:00', type: 'hotel', title: `${dest}市区酒店入住`, cost: 150, cat: 'hotel', detail: '市中心交通便利，150元/晚' },
        { time: '15:30', type: 'sight', title: `${dest}市区地标打卡`, cat: 'other', detail: '老城街区+夜市' },
    ];
    items.push(firstDay);
    for (let d = 2; d <= Math.max(2, days - 1); d++) {
        items.push([
            { time: '09:00', type: 'sight', title: `${dest}核心景点（第${d}天）`, cat: 'other', detail: '提前在官方渠道购票，早去避开人流' },
            { time: '12:00', type: 'food', title: `${dest}本地特色午餐`, cost: 50, cat: 'food', detail: '大众点评高分店，人均50元' },
            { time: '14:00', type: 'sight', title: `${dest}周边景点/博物馆`, cat: 'other', detail: '室内外结合，视天气调整' },
        ]);
    }
    items.push([
        { time: '09:00', type: 'sight', title: `${dest}剩余景点/自由活动`, cat: 'other', detail: '逛街买伴手礼' },
        { time: '14:00', type: 'transport_main', title: `${dest}→出发地（返程）`, cost: 200, cat: 'transport', detail: '按返程车次时间安排' },
    ]);
    return items;
}
function buildMockPlans(input) {
    ...
    const plans = variants.map((v) => {
        ...
        const daily = rawDays.map((dayItems, idx) => {
            const items = dayItems.map((it) => {
                let cost = it.cost;
                if (cost) {
                    if (it.type === 'transport_main' || it.type === 'transport_transfer')
                        cost = Math.round(cost * v.transM * budgetLevel);
                    else if (it.type === 'hotel')
                        cost = Math.round(cost * v.hotelM * budgetLevel);
                    else
                        cost = Math.round(cost * budgetLevel);
                }
                const cat = it.cat;
                if (cost) {
                    totalCost += cost;
    ...
    const peopleMap = { single: '单人', couple: '情侣', family: '家庭', friends: '朋友' };
...
//# sourceMappingURL=ai.mock.js.map